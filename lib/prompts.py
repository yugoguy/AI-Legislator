...
TOOL_GUIDANCE = (
    ...
    "source.\n"
    "- SearchWeb: search the web and get back REAL pages (titles + URLs + "
    "summaries). This is how you DISCOVER {region}'s own 議会議事録, 請願・陳情, "
    "パブリックコメント, 予算・決算, 計画 documents and any other local material. "
    "Always search first — never invent a URL.\n"
    "- FetchWebPage: fetch the full text of ONE specific URL. Only use it on a "
    "URL that SearchWeb (or another source) actually returned. Do NOT guess URLs; "
    "a guessed URL that fails is a dead end, not evidence.\n"
    ...
    "A failed tool call (HTTP error, page not found, no results) is feedback "
    "about your RESEARCH PROCESS — try a different query or source. It is never "
    ...
    "one. Record WHERE each fact came from (source name, and URL when applicable) "
    "so it can be cited in the proposal."
)
...
BUILD_PROPOSAL_SYSTEM = (
    ...
    "- 出典: citations for every factual claim\n"
    "CITATION QUALITY IS CRITICAL. Every external fact, statistic, or quotation "
    "in the proposal must be traceable to its origin: name the source (e.g. "
    "e-Stat table name/ID, 国会会議録 date+meeting+speaker, or the page title AND "
    "its URL for web material) inline or in 出典. Do not state a figure without "
    "saying where it came from. Never fabricate a statistic, a source, or a URL; "
    "if you do not have a real source for a claim, omit the claim or mark it "
    "explicitly as an unverified assumption. A failed/blocked tool result is NOT "
    "a source and must never be cited. If the evidence is thin, say so plainly "
    "rather than padding with unsourced generalities. Reference figures by "
...
WRITEUP_USER = (
    ...
    "sections: title (EN+JA), 本文, 提案理由, 出典. Every factual claim must carry "
    "a real, traceable citation in 出典 (source name, and page title + URL for "
    "web material); never invent a source or URL, and never cite a failed tool "
    "result. Reference any figures by their assets/ path.\n"
